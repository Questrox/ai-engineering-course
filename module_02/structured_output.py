...
class Ticket(BaseModel):
    ...
    category: Literal["регламент", "доступ", "инцидент", "документация"] = Field(
        description=("Это категория обращения: регламент - вопрос по регламенту, инструкции, порядку действий,"
        "доступ - заявка на доступ к системе, папке, оборудованию,"
        "инцидент - оборудование сломалось, остановилось, работает неправильно,"
        "документация - просьба найти или прислать документ, схему, паспорт"
        )
    )
    ...
    equipment_id: Optional[str] = Field(
        default=None,
        description="Идентификатор оборудования строго как в реестре предприятия: "
                    "КМ-101, НМ-205, П-7, ЭЛОУ-АВТ-6, Линия-3. Приведи описание к такому виду. "
                    "Если оборудовние в обращении не названо - null. Не угадывай и не выводи " 
                    "идентификатор из общих слов вроде \"станок\" или \"в цеху\"",
    )

    priority: Literal["низкий", "средний", "высокий"] = Field(
        description="высокий - остановлено производство или есть риск для людей," \
        "средний - мешает работе, но обходной путь есть," \
        "низкий - работе не мешает или практически не мешает, то, что может подождать." \
        "Оценивай по фактам, а не по эмоциям или восклицательным знакам."
    )
    ...
    def check_equipment_id(cls, value: Optional[str]) -> Optional[str]:
        ...
        raise ValueError(f"оборудование \"{value}\" отсутствует в реестре - заявка на ручную проверку")
